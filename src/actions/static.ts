...
const createResourceActions = (namespace: string) => {
  ...
  const actions: ResourceActions = {
    ...
    setCreateError: (error: any) => ({ type: types.CREATE_ERROR, error }),
    ...
    setUpdateError: (error: any) => ({ type: types.UPDATE_ERROR, error }),
    ...
    setRemoveError: (error: any) => ({ type: types.REMOVE_ERROR, error }),
